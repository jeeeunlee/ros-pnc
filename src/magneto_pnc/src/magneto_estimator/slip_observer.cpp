...
SlipObserver::SlipObserver( MagnetoControlSpecContainer* ws_container,
  ...
  pnc_utils::pretty_constructor(2, "StateEstimator: SlipObserver");
...
void SlipObserver::initialization(const YAML::Node& node) {
    try { 
        pnc_utils::readParameter(node,"slip_velocity_threshold", lin_vel_thres_);  
        pnc_utils::readParameter(node,"weight_shaping", weight_shaping_activated_); 
        pnc_utils::readParameter(node,"online_param_estimation", online_param_estimation_activated_); 
        pnc_utils::readParameter(node,"lpf_vel_cutoff", lpf_vel_cutoff_);
...
void SlipObserver::checkForce() {
    ...
    for( auto &[foot_idx, b_contact] : b_foot_contact_map_ ) {
        ...
        // pnc_utils::pretty_print(grf_act_des, std::cout, "grf_act_des"); 
        pnc_utils::saveVector(grf_act_des, filename);
    }
}
        ...
        if( foot_idx!=swing_foot_idx_ ) {
            ...
            if( fabs(xcdot[5]) < 0.001 && xcdot.segment(3,2).norm() > lin_vel_thres_ ) {
                ...
                if( stacked_grf_map_[foot_idx].size() == time_sampling_period_){      
                    ...
                    pnc_utils::saveVector(t_x0, foot_param_name);
...
Eigen::VectorXd SlipObserver::computeGRFDesired(const Eigen::VectorXd& tau) {    
    ...
    pnc_utils::pseudoInverse(A, 0.0001, Ainv);
