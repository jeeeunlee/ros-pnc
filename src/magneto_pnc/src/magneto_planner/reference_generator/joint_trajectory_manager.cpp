...
JointTrajectoryManager::JointTrajectoryManager(RobotSystem* _robot)
  ...
  pnc_utils::pretty_constructor(2, "TrajectoryManager: JointPos");
...
void JointTrajectoryManager::updateJointTrajectory(const double& current_time) {
  ...
  for (int i = 0; i < joint_dim_; ++i) {
    joint_pos_des_[i] = pnc_utils::smooth_changing(
        ini_jpos_[i], target_jpos_[i], traj_duration_, traj_time);
    joint_vel_des_[i] = pnc_utils::smooth_changing_vel(
        ini_jpos_[i], target_jpos_[i], traj_duration_, traj_time);
    joint_acc_des_[i] = pnc_utils::smooth_changing_acc(
        ini_jpos_[i], target_jpos_[i], traj_duration_, traj_time);
  }
}
