...
void MagnetoCoMPlanner::planCentroidalMotion(const Eigen::Vector3d& pcom_goal,
    ...
    // pnc_utils::saveValue(cpu_time_used, "cpu_time_used" );
    // std::cout<<"########################################################"<<std::endl;
}
...
void MagnetoCoMPlanner::_setConfigurations(const Eigen::Vector3d& pcom_goal,
    ...
    // pnc_utils::pretty_print(p_init_, std::cout, "p_init_");
    // pnc_utils::pretty_print(p_goal_, std::cout, "p_goal_");
...
void MagnetoCoMPlanner::_buildCentroidalSystemMatrices(){
    ...
    Ac_.topRows(3) = Pc_-pnc_utils::skew(p_goal_)*Rc_;
    Ac_.bottomRows(3) = Rc_;

    Cc_ = Eigen::VectorXd::Zero(6);
    Cc_.head(3) = Ldot_ + (pnc_utils::skew(p_goal_)*Rc_-Pc_)*Fmc_;
    Cc_.tail(3) = -mass_*grav_ - Rc_*Fmc_;

    dim_f = Rf_.cols();
    Af_ = Eigen::MatrixXd::Zero(6, dim_f);
    Af_.topRows(3) = Pf_-pnc_utils::skew(p_goal_)*Rf_;
    Af_.bottomRows(3) = Rf_;

    Cf_ = Eigen::VectorXd::Zero(6);
    Cf_.head(3) = Ldot_ + (pnc_utils::skew(p_goal_)*Rf_-Pf_)*Fmf_;
...
void MagnetoCoMPlanner::_buildFm(
    ...
    for(auto &mag : f_mag) {
        Fmf_= pnc_utils::vStack(Fmf_, -mag->getMagneticForce3d());
        if(mag->getLinkIdx() !=  swing_foot_link_idx_){
            Fmc_ = pnc_utils::vStack(Fmc_, -mag->getMagneticForce3d());
        }
    }    
}
...
void MagnetoCoMPlanner::_buildFrictionCone(
    ...
    for(auto &contact : f_contacts) {
        ...
        di = di.head(5);
        Di(1, 0) = 1.;
        Df_ = pnc_utils::dStack(Df_, Di);
        df_ = pnc_utils::vStack(df_, di);
        if(contact->getLinkIdx() != swing_foot_link_idx_){
            Dc_ = pnc_utils::dStack(Dc_, Di);
            dc_ = pnc_utils::vStack(dc_, di);
        }
    }
}
...
void MagnetoCoMPlanner::_buildWeightMatrices(
    ...
    for(auto &contact : f_contacts) {
        ...
        // invwi << mu*mu, mu*mu, 1.;
        invwi << mu*mu, mu*mu, 1.;
        invWf_ = pnc_utils::dStack(invWf_, invwi.asDiagonal());
        if(contact->getLinkIdx() != swing_foot_link_idx_){
            invWc_ = pnc_utils::dStack(invWc_, invwi.asDiagonal());
        }
    }
}
...
void MagnetoCoMPlanner::_buildPfRf() {
    ...
    for(int i(0); i<Magneto::n_leg; ++i) {        
        ...
        PRi = pnc_utils::skew(pi)*Ri;
        
        if(MagnetoFoot::LinkIdx[i] != swing_foot_link_idx_){
            Pf_ = pnc_utils::hStack(Pf_, PRi);
            Rf_ = pnc_utils::hStack(Rf_, Ri);
            Pc_ = pnc_utils::hStack(Pc_, PRi);
            Rc_ = pnc_utils::hStack(Rc_, Ri);
        }
        else{
...
void MagnetoCoMPlanner::_getSwingConditionReplan(Eigen::MatrixXd& DD,
    ...
    pnc_utils::pseudoInverse(AWA, 0.0001, AWAinv);
    ...
    // when t=Ts
    // DD2*alpha*dir >= ddf
    DD2 = Df_*(0.5*mass_*(T2_*T2_)*invA1*pnc_utils::skew(grav_) + mass_*invA2);

    DD = pnc_utils::vStack(DD1,DD2);
    dd = pnc_utils::vStack(ddf,ddf);    
}
...
void MagnetoCoMPlanner::_getEndTransitionCondition(Eigen::MatrixXd& DD,
    ...
    // when t=0
    // DD1*beta*dir - ddf >=0
    DD1 = Df_*( 0.5*mass_*(T3_*T3_)*invA1*pnc_utils::skew(grav_) + mass_*invA2);
    // when t=Tt
    // DD2*beta*dir - ddf >=0
    DD2 = Df_*mass_*invA2;
...
void MagnetoCoMPlanner::_getSwingConditionGivenRatio(double ratio,
    ...
    // @ t=T2
    DDa = pnc_utils::vStack(DDa, _computeSwingDDa(T2_, invA1, invA2));
    DDb = pnc_utils::vStack(DDb, _computeSwingDDb(T2_, invA1));
    dd = pnc_utils::vStack(ddc, ddc);

    // @ t=tstar (local maxima)
    double t_star = T2_ + T3_/2./ratio;
    if(t_star > 0. && t_star < T2_){
        DDa = pnc_utils::vStack(DDa, _computeSwingDDa(t_star, invA1, invA2));
        DDb = pnc_utils::vStack(DDb, _computeSwingDDb(t_star, invA1));
        dd = pnc_utils::vStack(dd, ddc);
    }
...
Eigen::MatrixXd MagnetoCoMPlanner::_computeSwingDDa(double t,
    ...
    return Dc_*( 0.5*mass_*(T2_-t)*(T2_-t)*invA1*pnc_utils::skew(grav_) + mass_*invA2);
}

Eigen::MatrixXd MagnetoCoMPlanner::_computeSwingDDb(double t,
                                            const Eigen::MatrixXd& invA1){
    // Db = D*( 0.5*m*T3*(T3+2*T2-t)*invA1*skew(g) );
    return Dc_*( 0.5*mass_*T3_*(T3_+2*T2_-t)*invA1*pnc_utils::skew(grav_) );
}
