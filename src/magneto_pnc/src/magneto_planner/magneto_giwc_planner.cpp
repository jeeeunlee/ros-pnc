// #include <magneto_pnc/ContactSet/ContactSet.hpp>
...
MagnetoGIWCPlanner::MagnetoGIWCPlanner(RobotSystem* robot) {
    pnc_utils::pretty_constructor(1, "Magneto GIWC Planner");
...
void MagnetoGIWCPlanner::setContact(const std::vector<ContactSpec*> & contact_list, 
    ...
    _ineqMat2Uf(Uf_);
    // pnc_utils::pretty_print(Uf_, std::cout, "Uf_i");
    dim_rows = Uf_.rows();
    dim_cols = Uf_.cols();
...
void MagnetoGIWCPlanner::_enforceJointLimit(const Eigen::VectorXd& q_curr, Eigen::VectorXd& dq) {
    ...
    // {
    //     pnc_utils::pretty_print(q_upper, std::cout, "q_upper");
    //     pnc_utils::pretty_print(q_curr, std::cout, "q_curr");
    //     pnc_utils::pretty_print(q_lower, std::cout, "q_lower");
    //     pnc_utils::pretty_print(dq, std::cout, "dq");
    //     pnc_utils::pretty_print(dq_temp, std::cout, "dq_temp");
                                      ...
                                      Eigen::Vector3d& com_pos_sample)
{
    ...
    // pnc_utils::pretty_print(zz, std::cout, "zz");
...
void MagnetoGIWCPlanner::_buildCoMJacobian()
{
    ...
    // pnc_utils::pretty_print(Jcom_, std::cout, "Jcom_");
    n_dof_ = Jcom_.cols();
}
...
void MagnetoGIWCPlanner::_buildContactJacobianNullspace(const std::vector<ContactSpec*> & contact_list)
{
    ...
    // pnc_utils::pretty_print(Nc_, std::cout, "Nc_");    
}
...
void MagnetoGIWCPlanner::_buildContactPlane(const std::vector<ContactSpec*> & contact_list)
{
    ...
    // pnc_utils::pretty_print(abc_contact_, std::cout, "abc_contact_");
    // pnc_utils::pretty_print(d_contact_, std::cout, "d_contact_");
}
...
{    
    ...
    // pnc_utils::pretty_print(Astance_, std::cout, "Astance_");
}
...
void MagnetoGIWCPlanner::_buildAstanceMatrixPoint(const std::vector<ContactSpec*> & contact_list)
{
    ...
    {        
        ...
        // pnc_utils::pretty_print(p_wb, std::cout, "p_wb");        
        // p_wb = p_wb - p_com;
...
void MagnetoGIWCPlanner::_buildHyperPlane(Eigen::MatrixXd& abc, Eigen::VectorXd& d) {
    ...
    {        
        ...
        // pnc_utils::pretty_print(gravity, std::cout, "gravity"); 
...
{
    Eigen::MatrixXd ret =  pnc_utils::getNullSpace(input, 0.0001);
    return ret;
}
...
{
    Eigen::MatrixXd ret;
    pnc_utils::pseudoInverse(input, 0.0001, ret);
    return ret;
}
