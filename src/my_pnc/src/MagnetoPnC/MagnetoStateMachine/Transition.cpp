...
#include <my_pnc/MagnetoPnC/MagnetoStateMachine/Transition.hpp>

Transition::Transition(const StateIdentifier state_identifier_in,
    MagnetoControlArchitecture* _ctrl_arch, RobotSystem* _robot, bool contact_start)
...
void Transition::firstVisit() {
  ...
  if(b_contact_start_){
    ...
    ctrl_arch_->taf_container_->set_magnetism(moving_foot_idx_); // off-magnetism on moving foot
    ctrl_arch_->taf_container_->set_residual_magnetic_force(moving_foot_idx_);
    ctrl_arch_->taf_container_->set_contact_magnetic_force(-1); // build full contact dim F_magnetic_    
  }
  ...
  //      CONTACT LIST
  // --------------------------------------- 
  ctrl_arch_->taf_container_->set_contact_list(-1);  // contain full contact
  ...
  if(b_contact_start_) {
    ...
    ctrl_arch_->max_normal_force_manager_
              ->setMaxNormalForceTrajectory(ctrl_start_time_, ctrl_duration_,
                                  ctrl_arch_->taf_container_->max_rf_z_nocontact_,
                                  ctrl_arch_->taf_container_->max_rf_z_contact_);
