...
MagnetoResidualController::MagnetoResidualController(
    MagnetoTaskAndForceContainer* _taf_container, RobotSystem* _robot)
    :MagnetoMainController(_taf_container, _robot) {
...
void MagnetoResidualController::_PreProcessing_Command() {
  ...
  wbrmc_param_->W_qddot_ = taf_container_->W_qddot_;
  wbrmc_param_->W_xddot_ = taf_container_->W_xddot_;
  wbrmc_param_->W_rf_ = taf_container_->W_rf_;
...
void MagnetoResidualController::firstVisit() { 
  
}

void MagnetoResidualController::ctrlInitialization(const YAML::Node& node) {
  // WBC Defaults
