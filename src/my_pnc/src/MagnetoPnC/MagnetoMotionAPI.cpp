...
MotionCommand::MotionCommand(TARGET_IDX _foot_idx,
                    const MOTION_DATA& _motion_data) {
    motion_sets_.clear();
    motion_sets_.insert(std::make_pair(_foot_idx, _motion_data));
}

// function
void MotionCommand::add_foot_motion(TARGET_IDX _foot_idx,
                                    const POSE_DATA& _pose_del,
                                    double _motion_period) {
    MOTION_DATA motion_data = MOTION_DATA(_pose_del, _motion_period);
    motion_sets_.insert(std::make_pair(_foot_idx, motion_data));
}
...
void MotionCommand::add_motion(TARGET_IDX _foot_idx,
                    const MOTION_DATA& _motion_data) {
    motion_sets_.insert(std::make_pair(_foot_idx, _motion_data));
}
...
void MotionCommand::clear_and_add_motion(TARGET_IDX _foot_idx,
                    const MOTION_DATA& _motion_data) {
    motion_sets_.clear();
    motion_sets_.insert(std::make_pair(_foot_idx, _motion_data));
}
...
bool MotionCommand::get_foot_motion_command(MOTION_DATA &_motion_data) {
 ...
 bool MotionCommand::get_foot_motion_command(MOTION_DATA &_motion_data, TARGET_IDX &_idx) {
    for(auto &[idx, motion_data] : motion_sets_) {
        if(idx > -1) {
            _idx = idx;
            _motion_data = motion_data;
