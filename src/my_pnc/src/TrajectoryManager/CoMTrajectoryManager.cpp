...
void CoMTrajectoryManager::setCoMTrajectory(double _start_time,
  ...
  // check print
  std::cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" <<std::endl;
  my_utils::pretty_print(pos_dev_b, std::cout, "com command!!!");
  
  MOTION_DATA motion_foot_data;
  Eigen::VectorXd pos_foot_dev_b;
  _motion_cmd->get_foot_motion_command(motion_foot_data);
  pos_foot_dev_b = 0.25 * motion_foot_data.pose.pos;
  my_utils::pretty_print(pos_foot_dev_b, std::cout, "foot command");
  std::cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" <<std::endl;
  ...
  // sp_->com_pos_target = com_pos_des_;
  // sp_->check_com_planner_updated ++; // draw plot on the simulation
}
