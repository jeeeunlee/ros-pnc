...
BasicTask::BasicTask(RobotSystem* _robot, const BasicTaskType& _taskType,
    ...
    pnc_utils::pretty_constructor(3, "Basic Task " + task_type_string_ + std::to_string(link_idx_));
}
...
bool BasicTask::_UpdateCommand(const Eigen::VectorXd& _pos_des,
    ...
    switch (task_type_) {
        case BasicTaskType::LINKORI: {
            ...
            for (int i = 0; i < 3; ++i) {
                // ori_err[i] = pnc_utils::bind_half_pi(ori_err[i]);
            }
            pos_err = ori_err;

            // vel_act
            vel_act = robot_->getBodyNodeCoMSpatialVelocity(link_idx_).head(3);
            // pnc_utils::pretty_print(pos_err, std::cout, "pos_err in ori");
            break;
        }
        ...
        case BasicTaskType::LINKXYZ: {
            ...
            //0112 pnc_utils::saveVector(pos_err, "pos_err");
            break;
        }
        case BasicTaskType::CENTROID: {
            // pos_err
        ...
        case BasicTaskType::COM: {
            ...
            // pnc_utils::pretty_print(pos_err, std::cout, "pos_err in COM");
            break;
        }
        default:
            std::cout << "[BasicTask] Type is not Specified" << std::endl;
...
bool BasicTask::_UpdateTaskJacobian() {
    switch (task_type_) {        
        ...
        case BasicTaskType::LINKXYZ: {
            ...
            // std::cout << "==================================" << std::endl;
            // pnc_utils::pretty_print(xdot, std::cout , "xdot");
            // pnc_utils::pretty_print(check_xdot_dev, std::cout , "check_xdot_dev");
            // std::cout << "---------------------------------" << std::endl;
...
bool BasicTask::_UpdateTaskJDotQdot() {
    switch (task_type_) {
        ...
        case BasicTaskType::LINKXYZ: {
            ...
            // pnc_utils::pretty_print(xddot, std::cout , "xddot");
            // pnc_utils::pretty_print(check_xddot_dev, std::cout , "check_xddot_dev");
