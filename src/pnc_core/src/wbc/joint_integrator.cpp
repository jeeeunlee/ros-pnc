...
JointIntegrator::JointIntegrator(const int num_joints_in,
                                 ...
                                 const double dt_in) {
  pnc_utils::pretty_constructor(3, "IHWBC Joint Integrator");
  n_joints_ = num_joints_in;
  vel_ = Eigen::VectorXd::Zero(n_joints_);
  pos_ = Eigen::VectorXd::Zero(n_joints_);
  setDt(dt_in);
...
void JointIntegrator::printIntegrationParams() {
  ...
  pnc_utils::pretty_print(vel_min_, std::cout, "velocity_min");
  pnc_utils::pretty_print(vel_max_, std::cout, "velocity_max");
  pnc_utils::pretty_print(pos_min_, std::cout, "position_min");
  pnc_utils::pretty_print(pos_max_, std::cout, "position_max");
  pnc_utils::pretty_print(pos_max_error_, std::cout, "position max error");

  pnc_utils::pretty_print(pos_, std::cout, "position current");
  pnc_utils::pretty_print(vel_, std::cout, "velocity current");
}
