...
class SystemParam{
    // x(k+1) = F*x(k) + B*u(k) + w(k), w(k)~N(0,Q)
    // z(k) = H*x(k) + v(k), v(k)~N(0,R)
    public:
        Eigen::MatrixXd F;
        Eigen::MatrixXd Q;
...
class KalmanFilter{
public:
	KalmanFilter() { b_initialize= false;}
	~KalmanFilter() {}
    ...
    void propagate(const Eigen::VectorXd& z, 
                   const Eigen::VectorXd& u, 
                    Eigen::VectorXd& xhat, 
                    void* system_data = NULL){
        if(system_data) sys_param_ = static_cast<SystemParam*>(system_data);

        xhat = x_hat_; // save prev
        predict(u); // update x_pre_, P_pre_
        update(z); // update x_hat_, P_hat_
    ...
    void predict(const Eigen::VectorXd& u) {
        x_pre_ =  sys_param_->F * x_hat_ + sys_param_->B * u;
        P_pre_ =  sys_param_->F * P_hat_ * sys_param_->F.transpose() + sys_param_->Q;
    }
